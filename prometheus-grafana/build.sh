...
kubectl create configmap --dry-run prometheus-rules \
...
echo "  " >> ./manifests/prometheus/prometheus-rules.yaml
...
# Workaround since `--namespace monitoring` from above is not preserved
echo " " >> ./manifests/alertmanager/alertmanager-templates.yaml
...
# Workaround since `--namespace monitoring` from above is not preserved
echo "  " >> ./manifests/grafana/import-dashboards/configmap.yaml
...
# Workaround since `--namespace monitoring` from above is not preserved
echo "  " >> ./manifests/prometheus/configmap.yaml
